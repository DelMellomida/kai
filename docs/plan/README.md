...
| Plan | What remains |
|---|---|
| [natural-audio-plan.md](wip/natural-audio-plan.md) | **Everything — written 2026-08-10, nothing implemented.** The second pass at "Kai sounds AI-generated", constrained to stay offline (the cloud option was offered and declined). Rests on three findings from reading the code: Piper's `noise_scale`/`noise_w` have **never been passed** (the shipping voice runs at stock VITS defaults, and `ai/tts.py` sends only `--length-scale`), the sox chain is loudness-only, and tempo varies per reply rather than per phrase. Also records that the previous hfc-vs-libritts A/B was confounded — those voices ship at 0.667/0.8 vs 0.333/0.333. Step 0 blocks the rest: it needs SSH access to the Jetson and a reproducible baseline. Read `completed/expressive-voice-plan.md` first, and § What this plan cannot do before promising anyone a result. |
